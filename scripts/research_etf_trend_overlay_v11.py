#!/usr/bin/env python3
"""Research V11: V10 beta hedge plus ETF trend sleeve.

The residual sleeve rotates among benchmark ETFs/cash/gold using point-in-time
trend and volatility. This is research only: production would still require
approved ETF/futures execution, position reconciliation, and capacity checks.

Default mode is the defensive cash/gold sleeve because the broad equity ETF
rotation increased drawdown in validation. Set
``QUANT_V11_TREND_ASSET_MODE=broad`` to reproduce the broader ETF experiment.
"""
...
def main() -> None:
    ...
    suffix = "defensive_overlay" if TREND_ASSET_MODE == "defensive" else "etf_trend_overlay"
    out_path = RESULTS_DIR / f"quant_logic_research_v11_{suffix}.json"
