...
CONFIG = {
    ...
    # Sector constraints
    "max_per_sector": 5,  # max 5 stocks per industry
    # Portfolio drawdown control (soft scaling, not hard stop)
    "dd_scale_enabled": True,
    "dd_threshold_1": 0.10,     # reduce to 75% when DD > 10%
    "dd_scale_1": 0.75,
    "dd_threshold_2": 0.20,     # reduce to 50% when DD > 20%
    "dd_scale_2": 0.50,
    "dd_threshold_3": 0.30,     # reduce to 25% when DD > 30%
    "dd_scale_3": 0.25,
}
...
def main():
    ...
    with open(OUT_DIR/"wf_backtest_v5.7_report.json","w") as f:
        json.dump({"ts":datetime.now().isoformat(),"version":"V5.7",
    ...
    print(f"\n{'='*60}\n  V5.4 — n=35, 90d + Sector Cap (max {CONFIG['max_per_sector']}/industry)")
