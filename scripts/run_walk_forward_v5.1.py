...
"""
V5.1 — WF validation of best V5 param combo: n=35, rebalance=45d, max_pct=0.20
"""
...
CONFIG = {
    "top_n": 35, "rebalance_freq": 45, "max_position_pct": 0.20,
...
def main():
    ...
    print(f"\nFull bt (n=35, 45d)...",flush=True); t1=time.time()
    ...
    g["A"]=g["S"] and g["D"] and g["W"]
    with open(OUT_DIR/"wf_backtest_v5.1_report.json","w") as f:
        json.dump({"ts":datetime.now().isoformat(),"version":"V5.1","desc":"n=35 rebalance=45d V3.5 MR",
                   "cfg":CONFIG,"full":full,"wf":w,"gates":g,"elapsed":round(time.time()-t0,1)},f,indent=2,default=str)
    print(f"\n{'='*60}\n  V5.1 — n=35, rebalance=45d, V3.5 MR factors")
    print(f"  {len(ca)} stocks, top-{CONFIG['top_n']}, {CONFIG['rebalance_freq']}d rebalance\n{'='*60}")
    if full:
        ...
        print(f"  Win={full['win_rate']:.1%} Active={full['avg_active_stocks']:.0f} Turnover={full['avg_turnover']:.1%}")
    for fld in w["folds"]:
    ...
    print(f"  {'✅' if g['A'] else '❌'} S={'✅' if g['S'] else '❌'} D={'✅' if g['D'] else '❌'} W={'✅' if g.get('W') else '❌'} M={'✅' if g['M'] else '❌'}")
