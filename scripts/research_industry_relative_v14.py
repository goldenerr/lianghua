#!/usr/bin/env python3
"""Research V14: regime-gated industry rotation plus industry-relative alpha.

This candidate keeps V13's regime-gated industry rotation, then blends each
stock's global score with its within-industry relative score. The goal is to
retain useful industry rotation while avoiding pure industry beta bets.
"""
...
CONFIG.update(
    {
        ...
        "industry_rotation_weight": 0.20,
        "industry_bear_rotation_weight": 0.40,
        "industry_top_normal": 24,
        "industry_top_bear": 8,
        "industry_min_members": 3,
        "industry_min_score": 0.0,
        "industry_filter_min_symbols": 80,
        "industry_risk_off_scale": 0.0,
        "industry_relative_weight": 0.35,
    }
)
...
def _apply_industry_rotation(
    ...
    rotation_weight = float(
        cfg["industry_bear_rotation_weight"] if bear else cfg["industry_rotation_weight"]
    )
    blend = (
        (1.0 - rotation_weight) * v9._safe_z(score)
        + rotation_weight * v9._safe_z(industry_factor)
    ).dropna()
    if not bear:
        return blend
...
def _industry_relative_blend(
    score: pd.Series,
    industry_map: dict[str, str],
    cfg: dict[str, float | int | str],
) -> pd.Series:
    weight = float(cfg["industry_relative_weight"])
    if score.empty or weight <= 0:
        return score
    industry = pd.Series(
        {
            symbol: industry_map.get(str(symbol), f"__UNKNOWN__:{symbol}")
    ...
    relative = pd.Series(index=score.index, dtype=float)
    for _, symbols in industry.groupby(industry).groups.items():
        group_score = score.reindex(symbols)
        z = v9._safe_z(group_score)
        relative.loc[z.index] = z
    blended = (1.0 - weight) * v9._safe_z(score) + weight * relative
    ...
    *,
    ...
    for t in range(start_idx + warmup, end_idx - 1):
        ...
        if (t - start_idx - warmup) % int(base_cfg["rebalance_freq"]) == 0:
            ...
            combined = _industry_relative_blend(combined, industry_map, cfg)
...
def main() -> None:
    ...
    report = {
        ...
        "version": "V14-industry-relative-alpha",
        "research_only": True,
    ...
    out_path = RESULTS_DIR / "quant_logic_research_v14_industry_relative.json"
