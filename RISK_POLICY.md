...
| --- | --- | --- |
| Local regression | Coverage `>=80%`, deterministic and safety tests | Met locally: `668` passing, `85.68%` coverage; Ruff/Black/mypy clean |
...
- The standard settlement-window `reduce_only` validator exists locally, but no approved production exchange-backed position provider is connected; configured source IDs without local bindings still require their downstream `data-001` approved provider implementations, and fee-model identifiers require `compliance-001` approved integrations; plugin strategies inherit the local market-session guard, but production still needs signed plugin loading/review evidence; the local holiday-calendar adapter still requires approved production operational evidence; and futures rollover execution remains intentionally unconnected pending approval and reconciliation controls.
