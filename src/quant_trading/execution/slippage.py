...
@dataclass
class SlippageModel:
    base_bps: float = 1.0; vol_sensitivity: float = 0.5; size_sensitivity: float = 0.1
    def estimate(self, volatility: float, order_size_pct: float) -> float:
        ...
        bps = self.base_bps + self.vol_sensitivity * volatility * 10000 + self.size_sensitivity * order_size_pct * 100
...
class ImpactModel:
    ...
    def estimate(self, volatility: float, order_shares: float, adv: float) -> float:
        if adv <= 0: return 0.0
        return round(self.kappa * volatility * np.sqrt(order_shares / adv), 6)
