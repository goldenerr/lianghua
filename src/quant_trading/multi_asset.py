...
ASSET_REGISTRY: dict[str, AssetSpec] = {
    # Crypto spot (Binance)
    "BTC/USDT": AssetSpec("BTC/USDT", "binance", AssetClass.SPOT, Market.CRYPTO,
                           tick_size=0.01, lot_size=1, min_notional=10,
                           maker_fee=0.001, taker_fee=0.001, max_leverage=1.0),
    "ETH/USDT": AssetSpec("ETH/USDT", "binance", AssetClass.SPOT, Market.CRYPTO,
                           tick_size=0.01, lot_size=1, min_notional=10,
                           maker_fee=0.001, taker_fee=0.001, max_leverage=1.0),
    "SOL/USDT": AssetSpec("SOL/USDT", "binance", AssetClass.SPOT, Market.CRYPTO,
                           tick_size=0.01, lot_size=1, min_notional=10),
    # Crypto perpetual futures
    "BTC/USDT:USDT": AssetSpec("BTC/USDT:USDT", "binance", AssetClass.PERPETUAL, Market.CRYPTO,
                                tick_size=0.1, lot_size=1, min_notional=10,
                                maker_fee=0.0002, taker_fee=0.0004, max_leverage=10.0,
                                funding_rate_interval=8),
    "ETH/USDT:USDT": AssetSpec("ETH/USDT:USDT", "binance", AssetClass.PERPETUAL, Market.CRYPTO,
                                tick_size=0.01, lot_size=1, min_notional=10,
                                maker_fee=0.0002, taker_fee=0.0004, max_leverage=10.0),
}

# A-share stocks pattern (instantiated dynamically)
ASHARE_SPEC = AssetSpec("", "sse/szse", AssetClass.SPOT, Market.ASHARE,
                         tick_size=0.01, lot_size=100, min_notional=100,
                         maker_fee=0.00025, taker_fee=0.00025,
                         max_leverage=1.0, trading_hours="9:30-11:30,13:00-15:00")
...
class MultiAssetDataProvider:
    ...
    def _fetch_ashare_ohlcv(self, asset: AssetSpec, limit: int):
        ...
        try:
            ...
            df = ak.stock_zh_a_hist(symbol=asset.symbol, period="daily",
            ...
            result = np.column_stack([
                ...
                df['开盘'], df['最高'], df['最低'], df['收盘'], df['成交量'],
            ])
            return result[-limit:]
        except Exception as e:
    ...
    def _fetch_commodity_ohlcv(self, asset: AssetSpec, limit: int):
        ...
        try:
            ...
            return np.column_stack([
                ...
                df['开盘价'], df['最高价'], df['最低价'], df['收盘价'], df['成交量'],
            ])[-limit:]
        except Exception:
            return None
...
class CrossAssetRiskConfig:
    ...
    max_exposure_pct: dict[AssetClass, float] = field(default_factory=lambda: {
        AssetClass.SPOT: 1.0,
    ...
    max_currency_exposure_pct: dict[str, float] = field(default_factory=lambda: {
        "USD": 0.5, "CNY": 1.0, "USDT": 0.3,
    })
    # Per-asset limits
    ...
    def update_position(self, symbol: str, asset: AssetSpec, side: str, 
        ...
        if side == 'buy':
            ...
            if pos['shares'] <= 0 < new_shares:
                ...
                pos['avg_price'] = (pos['shares'] * pos['avg_price'] + quantity * price) / new_shares
                pos['shares'] = new_shares
            else:
                # Adding to short
                pos['shares'] = new_shares
            ...
            if pos['shares'] >= 0 > new_shares:
                ...
                pos['avg_price'] = (abs(pos['shares']) * pos['avg_price'] + quantity * price) / abs(new_shares)
                pos['shares'] = new_shares
            else:
                pos['shares'] = new_shares
                pos['avg_price'] = price
