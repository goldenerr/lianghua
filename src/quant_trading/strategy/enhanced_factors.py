...
def factor_pe_percentile(pe_value: float | None) -> float:
    ...
    if pe_value is None or np.isnan(pe_value) or pe_value <= 0:
        return np.nan
    return -np.log(pe_value)  # log scale: lower PE → higher score
...
def factor_volume_price_trend(closes: np.ndarray, volumes: np.ndarray,
                               window: int = 5) -> float:
...
def factor_relative_strength(closes: np.ndarray, benchmark_closes: np.ndarray | None = None,
                              window: int = 63) -> float:
