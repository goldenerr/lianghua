...
def factor_rsi(closes: np.ndarray, period: int = 14) -> float:
    ...
    if loss_avg < 1e-12:
        return np.nan
    return float(-abs(50.0 - 100.0 / (1.0 + g / loss_avg)))
...
def log_market_cap_proxy(amounts: np.ndarray, turnover: np.ndarray) -> float:
    ...
    if len(amounts) < 21 or len(turnover) < 21:
        return np.nan
...
def chaikin_money_flow(
    highs: np.ndarray, lows: np.ndarray, closes: np.ndarray, volumes: np.ndarray, window: int = 20
) -> float:
    ...
    hl_diff = np.where(h - low_values < 1e-12, 1e-12, h - low_values)
    mfm = ((c - low_values) - (h - c)) / hl_diff
    mfv = mfm * v
...
def compute_factor(spec: FactorSpec, data: dict[str, Any]) -> float:
    """Compute a single factor from stock data dict."""
    try:
        args = [
            (
                np.asarray(data[col])
                if col in data and not isinstance(data[col], int | float)
                else data.get(col)
            )
            for col in spec.data_cols
        ]
        return float(spec.func(*args))
    except (ValueError, TypeError, KeyError):
        return np.nan


def compute_all_factors(
    stock_data: dict[str, Any], factor_names: list[str] | None = None
) -> dict[str, float]:
    """Compute all specified factors for one stock."""
    names = factor_names or FACTOR_NAMES
    return {
        name: compute_factor(FACTOR_BY_NAME[name], stock_data)
        for name in names
