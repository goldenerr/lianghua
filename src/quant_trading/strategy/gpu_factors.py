...
def _rsi_numba(closes, period=14):
...
@jit(nopython=True, cache=True)
def _momentum_numba(closes, lookback=63, skip=5):
    """Numba JIT momentum."""
...
@jit(nopython=True, cache=True)
def _bollinger_pos_numba(closes, window=20):
    ...
    if width < 1e-12:
        return 0.0
    return -abs(closes[-1] - m) / width


@jit(nopython=True, cache=True)
def _vol_numba(closes, window=21):
...
@jit(nopython=True, cache=True)
def _turnover_numba(volumes, window=21):
...
@jit(nopython=True, cache=True)
def _reversal_numba(closes, lookback=5):
    """Numba JIT reversal."""
...
@jit(nopython=True, cache=True)
def _macd_hist_numba(closes, fast=12, slow=26, signal=9):
...
@jit(nopython=True, cache=True)
def _amihud_numba(closes, amounts, window=21):
...
@jit(nopython=True, cache=True)
def _max_ret_numba(closes, window=21):
...
def compute_factor_matrix_cpu(closes_all, volumes_all, amounts_all):
