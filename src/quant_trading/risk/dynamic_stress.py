...
class MarketRegime:
    ...
    def is_stressed(self) -> bool:
        return self.volatility > 0.03 or self.liquidity < 0.3


def detect_regime(returns: np.ndarray, volumes: np.ndarray | None = None) -> MarketRegime:
    ...
    liq = (
        1.0
        if volumes is None
        else float(np.median(volumes) / (np.median(volumes) + np.std(volumes)))
    )
...
def generate_regime_scenarios(
    ...
    rng = np.random.RandomState(
        seed if seed is not None else int(datetime.now(timezone.utc).timestamp()) % 10000
    )
    scenarios = []
